...
/**
 * The 27 hand-authored items from the original hardcoded [com.example.data.model.IsaacItemDatabase].
 * Preserved verbatim so the curated [SynergyInfo] content is not lost when the app switches to the
 * bundled 721-item catalog. Only the [IsaacItem.synergies] lists are overlaid onto catalog items
 * (matched case-insensitively by name); the rest of each entry here is historical reference.
 */
object CuratedSynergies {

    val curatedItems: List<IsaacItem> = listOf(
    ...
    /** name (lowercased, trimmed) -> curated synergy list. */
    val byName: Map<String, List<SynergyInfo>> =
        curatedItems.associate { it.name.trim().lowercase() to it.synergies }
}
